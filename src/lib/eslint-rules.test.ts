...
describe('.eslintrc.json enforcement', () => {
  ...
  beforeAll(() => {
    // useEslintrc:true -> resolve the repo-root .eslintrc.json hierarchy.
    eslint = new ESLint({ cwd: repoRoot, useEslintrc: true });
  });

  it('flags a raw console.* as no-console at severity 2 (ERROR)', async () => {
  ...
  it('flags a raw toLocaleString as no-restricted-syntax at severity 1 (WARN)', async () => {
